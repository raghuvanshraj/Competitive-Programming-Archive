/**
 *    author:	vulkan
 *    created:	26.03.2020 08:21:25 PM
**/
#include <bits/stdc++.h>
...
bool solve(vector<vector<int>> &adj_list, set<int> &vert, int u, int p) {
	if (vert.empty()) {
		return true;
	}

	vector<int> temp;
	if (vert.find(u) != vert.end()) {
		temp.push_back(u);
		vert.erase(u);
	}
...
int main(int argc, char const *argv[]) {
	...
	while (m--) {
		...
		string ans = solve(adj_list, vert, 0, -1) ? "YES" : "NO";
		cout << ans << endl;
	}
