/**
 *    author:	vulkan
 *    created:	19.02.2020 03:48:35 PM
**/
#include <bits/stdc++.h>
...
int main(int argc, char const *argv[]) {
	...
	vector<int> start_here(n), end_here(n);

	end_here[0] = 1;
	for (int i = 1; i < n; ++i) {
		if (arr[i] > arr[i - 1]) {
			end_here[i] = end_here[i - 1] + 1;
		} else {
			end_here[i] = 1;
		}
	}
	...
	for (int i = 1; i < n - 1; ++i) {
		if (arr[i + 1] > arr[i - 1]) {
			ans = max(ans, end_here[i - 1] + start_here[i + 1]);
		}

		ans = max(ans, end_here[i] + start_here[i] - 1);
	}
