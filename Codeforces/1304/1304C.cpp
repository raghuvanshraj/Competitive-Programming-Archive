/**
 *    author:	vulkan
 *    created:	20.02.2020 02:05:14 PM
**/
#include <bits/stdc++.h>
...
int main(int argc, char const *argv[]) {
	...
	while (q--) {
		int n, m;
		cin >> n >> m;
		vector<customer_t> customers(n);
		for (int i = 0; i < n; ++i) {
			cin >> customers[i].time >> customers[i].low >> customers[i].high;
		}
		...
		bool possible = true;
		for (int i = 0; i < n; ++i) {
			int time_delta = customers[i].time - curr_t;
			low -= time_delta;
			high += time_delta;
			if (low > customers[i].high or high < customers[i].low) {
				possible = false;
				break;
			}

			curr_t += time_delta;
			low = max(low, customers[i].low);
			high = min(high, customers[i].high);
		}
