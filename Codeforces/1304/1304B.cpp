/**
 *    author:	vulkan
 *    created:	20.02.2020 01:34:03 PM
**/
#include <bits/stdc++.h>
...
int main(int argc, char const *argv[]) {
	...
	for (int i = 0; i < n; ++i) {
		...
		string temp(m, ' ');
		reverse_copy(strings[i].begin(), strings[i].end(), temp.begin());
		for (int j = 0; j < n; ++j) {
			if (i != j and not visited[j] and temp == strings[j]) {
				visited[i] = visited[j] = true;
				string_pairs.pb({strings[i], strings[j]});
			}
		}
	}
	...
	for (int i = 0; i < n; ++i) {
		if (not visited[i] and max_len < strings[i].size()) {
			string temp(m, ' ');
			reverse_copy(strings[i].begin(), strings[i].end(), temp.begin());
			if (temp == strings[i]) {
	...
	string ans = (max_len_idx == -1) ? "" : strings[max_len_idx];
	for (int i = 0; i < string_pairs.size(); ++i) {
		ans = string_pairs[i].ff + ans + string_pairs[i].ss;
	}
