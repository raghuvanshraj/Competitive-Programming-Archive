/**
 *    author:	vulkan
 *    created:	17.03.2020 11:39:46 AM
**/
#include <bits/stdc++.h>
...
double get_minimum_radius(vector<int> &pos, int l) {
	int n = pos.size();

	double max_dist = 0.0;
	sort(all(pos));
	for (int i = 1; i < n; ++i) {
		max_dist = max(max_dist, double(pos[i] - pos[i - 1]));
	}
...
int main(int argc, char const *argv[]) {
	...
	double ans = get_minimum_radius(pos, l);
	printf("%.10f\n", ans);
