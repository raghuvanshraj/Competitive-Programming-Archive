/**
 *    author:	vulkan
 *    created:	09.09.2020 12:12:57 PM
**/
#include <bits/stdc++.h>
...
void segment_tree_t::build(int x, int lx, int rx) {
	if (lx == rx) {
		this->tree[x] = {1, 1};
		return;
	}
	this->tree[x] = {1, (rx - lx + 1)};
	int mx = (lx + rx) / 2;
	this->build(2 * x, lx, mx);
	this->build(2 * x + 1, mx + 1, rx);
}
...
void segment_tree_t::_update(int l, int r, LL v, int x, int lx, int rx) {
	if (rx < l or lx > r) {
		...
		this->tree[x].first = (this->tree[x].first * v) % MOD;
		this->tree[x].second = (this->tree[x].second * v) % MOD;
		return;
	}
	...
	this->tree[x].second = (((this->tree[2 * x].second + this->tree[2 * x + 1].second) % MOD) * this->tree[x].first) % MOD;
}
...
LL segment_tree_t::_query(int l, int r, int x, int lx, int rx) {
	...
	int mx = (lx + rx) / 2;
	return (((this->_query(l, r, 2 * x, lx, mx) + this->_query(l, r, 2 * x + 1, mx + 1, rx)) % MOD) * this->tree[x].first) % MOD;
}
