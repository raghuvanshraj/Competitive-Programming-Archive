/**
 *    author:	vulkan
 *    created:	09.09.2020 12:32:03 PM
**/
#include <bits/stdc++.h>
...
class segment_tree_t {
	...
	int _query(int l, int r, int x, int lx, int rx);
public:
...
segment_tree_t::segment_tree_t(int n) {
	...
	this->ops = vector<int>(2 * this->n);
	this->ans = vector<int>(2 * this->n);
}
...
void segment_tree_t::_update(int l, int r, int v, int x, int lx, int rx) {
	...
	this->ans[x] = ((this->ans[2 * x] & this->ans[2 * x + 1]) | this->ops[x]);
}
...
int segment_tree_t::_query(int l, int r, int x, int lx, int rx) {
	if (rx < l or lx > r) {
		return ((1 << 30) - 1);
	} else if (lx >= l and rx <= r) {
		return this->ans[x];
	...
	int mx = (lx + rx) / 2;
	int lft = this->_query(l, r, 2 * x, lx, mx);
	int rgt = this->_query(l, r, 2 * x + 1, mx + 1, rx);
	return ((lft & rgt) | this->ops[x]);
}
